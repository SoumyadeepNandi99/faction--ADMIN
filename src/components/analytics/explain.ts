...
export const EXPLAIN = {
    ...
    mostActive: "Students ranked by time spent solving questions (the sum of per-question time taken) over the selected window, defaulting to the last 10 days. This is measured time-on-task while solving, not full app screentime, which the platform does not track. Each attempt's time is capped at 10 minutes to remove outliers from tabs left open. Also shows the single most active student for each day.",
} as const;
